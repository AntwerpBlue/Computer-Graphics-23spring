...
float recRotation[4] = { 1,0,0,0 };

int mode = 1;//1 for uni-weighted and 2 for cot-weighted


void deform_preprocess(int mode)
{
    auto L = Laplacian(meshX, meshF, mode);
    eigen2matlab("L", L);
...
void rotationcallback(int) {
    if (myRotation[0] != recRotation[0] || myRotation[1] != recRotation[1] || myRotation[2] != recRotation[2] || myRotation[3] != recRotation[3]) {
...
int main(int argc, char *argv[])
{
    ...
    std::cout << "Enter 1 for Uni-weighted, 2 for Cot-weighted, defalt 1:";
    std::cin >> mode;


    const char* meshpath = argc > 1 ? argv[1] : "balls.obj";
    readObj(meshpath, x, f);//x for vertices and f for faces
