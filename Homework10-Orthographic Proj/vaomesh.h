...
struct GLMesh
{
    ...
    Eigen::Matrix4f orthographic(float left,float right,float aspect, float zNear, float zFar) const
    {
        assert(zFar > zNear);
    ...
    std::array<R, 16> matMVP(const int *vp, bool offsetDepth = false, bool colmajor = false) const {
        ...
        Mat4 proj = orthographic(-2.0,2.0,vp[2]/float(vp[3]), -10.0, 10.0);
